import { typography } from '../../config/theme';
// Yoiden light theme — blue & white branding
export const T = {
  bg: '#FFFFFF', bgSoft: '#F5F7FA', bgDeep: '#EEF2F7',
  navy: '#001E40', blue: '#1858D6',
  card: '#FFFFFF', line: '#E2E8F0',
  ink: '#1A1D21', muted: '#64748B', faint: '#94A3B8',
  onNavy: '#FFFFFF', win: '#0F9D63', winBg: '#E7F8F1', gold: '#B45309',
  winStrong: '#0F9D63', winBg2: '#DCFCE7', accentBlue: '#1858D6', navyDeep: '#00132B',
  head: typography.fontFamily.headline, body: typography.fontFamily.body,
};
