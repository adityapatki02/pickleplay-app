...
const TEXT_SECONDARY = YColors.ink;
const BORDER_COLOR = '#E2E8F0';
const INPUT_BG = '#F5F7FA';
const CARD_BG = '#FFFFFF';
const BG_WHITE = YColors.bg;
...
function getRoundLabel(round: string | number | null | undefined): string {
  if (round == null) return 'POOL PLAY';
  // Non-numeric strings (e.g. "pool", "qf") — show uppercase
  if (typeof round === 'string' && !/^-?\d+$/.test(round.trim())) {
    return round.toUpperCase().replace(/_/g, ' ');
  }
  const n = typeof round === 'string' ? parseInt(round, 10) : round;
  if (Number.isNaN(n as number)) return 'POOL PLAY';
  if (n === -1) return 'FINAL';
  if (n === -2) return 'SEMI-FINALS';
  if (n === -3) return 'QUARTER-FINALS';
  if ((n as number) < 0) return `ROUND ${Math.abs(n as number)}`;
  return `ROUND ${n}`;
}

function getMaxGames(matchFormat?: string): number {
  if (matchFormat === 'best_of_5') return 5;
  if (matchFormat === 'best_of_3') return 3;
  if (matchFormat === 'best_of_1' || matchFormat === 'single_game') return 1;
  return 1; // safe default — single game
}
...
export default function ScoreEntryScreen() {
  ...
  // Single match mode: matchId takes precedence regardless of whether
  // tournamentId is also passed (Schedule passes both).
  const [singleMatch, setSingleMatch] = useState<MatchItem | null>(null);

  const isSingleMatchMode = Boolean(matchId);
    ...
    if (loading) {
      return (
        <SafeAreaView style={styles.safeArea}>
          ...
          <YTopBar eyebrow="ORGANIZER" title="SCORE ENTRY" onBack={() => navigation.goBack()} />
